...
pub extern "C" fn pam_sm_open_session(pamh: &module::PamHandleT,
    ...
    let log_err_and_return = |(e, msg): (PamResultCode, Cow<str>)| -> PamResultCode {
        if e != PAM_SUCCESS {
            ...
            let _ = syslog::unix(Facility::LOG_AUTH)
                .and_then(|writer| {
                    writer.send_3164(Severity::LOG_ALERT,
    ...
    open_session(pamh, flags, args)
            .unwrap_or_else(log_err_and_return)
}
...
pub fn open_session(pamh: &module::PamHandleT,
    ...
    let username = module::get_user(pamh, None)
        .map_err(|e| (e, Cow::from("")))?;
